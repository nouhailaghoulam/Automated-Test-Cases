...
33194
    ...
    Sleep    10
    Comment etape 12
    Je clique sur iframe[name="${iframe}"] >>> div[field="SBQQ__ProductName__c"]:has(:text("AMO-AC")) >> xpath=following-sibling::div[contains(@class, "drawerContainer")]
    Je clique sur iframe[name="${iframe}"] >>> div.container:has(:text("Modification manuelle")) >> div[id="formatted"]
    Je clique sur iframe[name="${iframe}"] >>> div[field="SBQQ__ProductName__c"]:has(:text("AMO-AC")) >> xpath=following-sibling::div[contains(@field, "SBQQ__ListPrice__c")]
    Je clique sur iframe[name="${iframe}"] >>> div.container:has(:text("Modification manuelle")) >> div[id="formatted"]
    Je clique sur iframe[name="${iframe}"] >>> div[field="SBQQ__ProductName__c"]:has(:text("AMO-AC")) >> xpath=following-sibling::div[contains(@field, "SBQQ__ListPrice__c")]
    Je clique sur iframe[name="${iframe}"] >>> div.container:has(:text("Modification manuelle")) >> div[id="formatted"]
    Je clique sur iframe[name="${iframe}"] >>> div[field="SBQQ__ProductName__c"]:has(:text("AMO-AC")) >> xpath=following-sibling::div[contains(@field, "SBQQ__ListPrice__c")]
    ...
    Keyboard Input    insertText    1500
    ...
    Je clique sur "Modifier Afficher les échéanciers par prestation" >> nth=0
    Je clique sur //input[@type='checkbox' and @name='GLOBAL_ShowBillingSchedule__c'] >> nth=1
    Je clique sur //button[@name='SaveEdit']
    Sleep    20
    Reload
    Je clique sur "Echéancier"
    ...
    Je verifie la presence de //tr[th[@data-cell-value='A la remise du rapport'] and td[@data-cell-value='0.5']]
    Comment etape 23
    ...
    Sleep    2
    Comment etape 23
    scroll to Element    //tr[th[@data-cell-value="A l'issue de l'intervention"]]
